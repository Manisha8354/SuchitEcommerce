...
export default function Products() {
  ...
  async function addCart(product) {
    if (auth.username) {
      let cartData = { ...product, productImages: `https://actl.co.in/suchit_uploads/${product.productImages[0]}` };
  ...
  async function addWish(product) {
    if (auth.username) {
      let wishData = { ...product, productImages: `https://actl.co.in/suchit_uploads/${product.productImages[0]}` };
      let user = auth.username.email.split('@')[0] + '_suchit_wish';
      await axios.post(`https://actl.co.in/suchit/wishSave/${user}`, wishData);
      navigation('/wishlist');
    } else {
      navigation('/signinsignup');
    }
