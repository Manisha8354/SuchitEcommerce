...
let {auth} = useContext(UserContext)
  async function getProfile() {
    try {
      let result = await axios.get(`https://actl.co.in/suchit/getProductByCode/${code}`);
      if (result) {
        let x = '';
        ...
        setData(final[0]);
        setMainImage(`https://actl.co.in/suchit_uploads/${final[0].productImages[0]}`); // Set the main image initially
      }
    } catch (error) {
  ...
  async function getAllProfile() {
    try {
      let result = await axios.get('https://actl.co.in/suchit/getProduct');
      if(result){
        const final = result.data.map(item => {
          if (typeof item.productImages === 'string') {
            return { ...item, productImages: JSON.parse(item.productImages),productSize: JSON.parse(item.productSize)  };
 ...
 async function addCart(data){
  // if(size){
    ...
    let user =  auth.username.email.split('@')[0] + '_suchit_cart'
    // console.log(user)
    await axios.post(`https://actl.co.in/suchit/cartSave/${user}`,cartdata)
    navigation('/cart')
    }else{
 ...
 async function addWish(data){
  // if(size){
    if(auth.username){
      let cartdata = {...data, productImages:mainImage}
    let user =  auth.username.email.split('@')[0] + '_suchit_wish'
    // console.log(user)
    await axios.post(`https://actl.co.in/suchit/wishSave/${user}`,cartdata)
    navigation('/wishlist')
    }else{
      navigation('/signinsignup')
    }
...
const handleCheckout = async (item) => {
  ...
  const cartData = [{
    ...
    productImages: `https://actl.co.in/suchit_uploads/${item.productImages[0]}`,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="flex flex-wrap gap-2 justify-center mt-4">
              ...
              {data.productImages.map((image, index) => (
                <img
                  ...
                  src={`https://actl.co.in/suchit_uploads/${image}`}
                  alt={`Product Thumbnail ${index + 1}`}
                  onClick={() => handleThumbnailClick(`https://actl.co.in/suchit_uploads/${image}`)} // Set image as main on click
                />
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-4">
              ...
              <button onClick={()=>addWish(data)} className="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-lg w-full sm:w-auto">
                Add to Wishlist
              </button>
              <button onClick={()=>handleCheckout(data)} className="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-lg w-full sm:w-auto">
                Buy Now
              </button>
            </div>
          </div>
    ...
    `}
        ...
        {product.productImages.map(slide => (
          <SwiperSlide key={slide.id}>
            <img
              src={`https://actl.co.in/suchit_uploads/${slide}`}
              alt={`Slide ${slide.id}`}
